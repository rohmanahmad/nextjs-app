...
export default function Header() {
  ...
  function closeNotificationsMenu(e) { }
  function toggleProfileMenu(e) { }
  function closeProfileMenu(e) { }
  const isNotificationOpen = false
  const isProfileMenuOpen = false
  const notifOpenTemplate = (
    <ul
    ...
    <header className="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
      <div className="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
        ...
        <ul className="flex items-center flex-shrink-0 space-x-6">
          {/* <!-- Theme toggler --> */}
          <li className="flex">
            <button className="rounded-md focus:outline-none focus:shadow-outline-purple" onClick={toggleTheme}
          ...
          <li className="relative">
            <button className="relative align-middle rounded-md focus:outline-none focus:shadow-outline-purple"
              onClick={toggleNotificationsMenu} onKeyDown={closeNotificationsMenu} aria-label="Notifications"
              aria-haspopup="true">
              {lightTemplate}
              {/* <!-- Notification badge --> */}
            ...
            {isNotificationOpen ? notifOpenTemplate : ''}
          </li>
          {/* <!-- Profile menu --> */}
          <li className="relative">
            <button className="align-middle rounded-full focus:shadow-outline-purple focus:outline-none"
              onClick={toggleProfileMenu} onKeyDown={closeProfileMenu} aria-label="Account" aria-haspopup="true">
              <img className="object-cover w-8 h-8 rounded-full"
                src="/assets/smart-hr/client/profile.jpeg"
                alt="" aria-hidden="true" />
            </button>
            {isProfileMenuOpen ? profileMenuTemplate : ''}
          </li>
        </ul>
      </div>
    </header>
